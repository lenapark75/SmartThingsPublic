...
@Field String google_cookiep1='__Secure-3PSID=_gdWcc7jxJqEzlGg4bVRVMGVGjwBww736FLkXXaZdvjmr2Am7N8TJEvaF17LNxG1uOkdnw.; __Secure-3PAPISID=mwDq6HVE03d3gvjw/AWJXzQxt8WkRYrCEY; NID=219=aGI5tFhc7w57CdFM9xEkwsNQ6uwMe6t9y_bmRyC_YkzK4vN9oVge3tSGim_YSXo2y8FwdayWTw76mXrfa0y6Bf-HazV8hOApaD1mL9iM3MQid5rXFeNcAuM9xdN6cTGvYbty6W7fSM9BhCHNecO7ig85ZfqOAYwuy2Ag2V3Dhof_GD_85jgrEliExHuBrl2UdZdO1KgxwuXZHw; __Host-3PLSID=s.KR|s.youtube:_gdWcQUOD1HPOq57DieIv5Nl-zuEU5D09tvxYlOh06eVvs4frVkbQ7qFsXdTN4vyGjHLNQ.; __Secure-3PSIDCC=AJi4QfGI3lCYwy8T8txB_k7JD2-q7JpIjeMdAgzccIXwqA-clV7Q_zurBUfcgRm8_-H9LhnvkQ'
@Field String google_issue_token_url='Request URL: https://accounts.google.com/o/oauth2/iframerpc?action=issueToken&response_type=token%20id_token&login_hint=AJDLj6LQu-SMCQLMJfOUep6S7tCex_yxKrsOFgFu38jKlSzaTTJaYTDKS7EFJtzZCEZaRQI8ZgM57UK9x8geSPHlngjUOvtd1w&client_id=733249279899-44tchle2kaa9afr5v9ov7jbuojfr9lrq.apps.googleusercontent.com&origin=https%3A%2F%2Fhome.nest.com&scope=openid%20profile%20email%20https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fnest-account&ss_domain=https%3A%2F%2Fhome.nest.com'
//Not needed anymore
...
// ** DO NOT USE THE AppSetting variables ANYMORE, LEFT FOR RETRIEVAL of OLD VALUES
{
...
// atomicState?.deviceData must be populated prior to the call
// sensorSN must be a valid serial number for a sensor

def getParentTstatForSensor(String sensorSN) {

	traceEvent(settings.logFilter,"getParentTstatForSensor>begin fetching ${sensorSN} data...", detailedNotif,GLOBAL_LOG_TRACE)
	
	def sensorData=[:]
	def thermostatId=null
	try {
		sensorData=atomicState?.deviceData[sensorSN]
		thermostatId=sensorData?.tstat_id    
		traceEvent(settings.logFilter,"getParentTstatForSensor>found ${sensorSN} in deviceData, data=${sensorData}", detailedNotif,GLOBAL_LOG_TRACE)
	} catch (e) {
		traceEvent(settings.logFilter,"getParentTstatForSensor>no sensor data for serial number $sensorSN...", detailedNotif,GLOBAL_LOG_WARN)
		return null        
	}    
	traceEvent(settings.logFilter,"getParentTstatForSensor>end...found ${thermostatId} for ${sensorSN} ...", detailedNotif,GLOBAL_LOG_TRACE)
	return thermostatId
}

// atomicState?.deviceData must be populated prior to the call
def getRCSAssociatedSensorsForTstat(String thermostatId) {

	traceEvent(settings.logFilter,"getRCSAssociatedSensorsForTstat>begin fetching sensors data for thermostatId $thermostatId...", detailedNotif,GLOBAL_LOG_TRACE)
	def thermostatData=[:]
	def listOfSensors=[]
	def deviceData=[:]
	deviceData=atomicState?.deviceData
	if (!deviceData) {
		traceEvent(settings.logFilter,"getRCSAssociatedSensorsForTstat>No device data available, exiting...", detailedNotif,GLOBAL_LOG_WARN)
    	return []
	}        
	try {    
		thermostatData=deviceData[thermostatId]
		listOfSensors= thermostatData?.associated_rcs_sensors
		if ((listOfSensors=="") || (listOfSensors==null))  listOfSensors=[]        
		traceEvent(settings.logFilter,"getRCSAssociatedSensorsForTstat>List of Sensors=$listOfSensors for serial number $thermostatId...", detailedNotif,GLOBAL_LOG_INFO)
	} catch (e) {
		traceEvent(settings.logFilter,"getRCSAssociatedSensorsForTstat>no thermostat data for serial number $thermostatId...", detailedNotif,GLOBAL_LOG_WARN)
	}    
	return listOfSensors    
}
// atomicState?.deviceData must be populated prior to the call
def getRCSActiveSensorsForTstat(String thermostatId) {

	traceEvent(settings.logFilter,"getRCSActiveSensorsForTstat>begin fetching sensors data for thermostatId $thermostatId...", detailedNotif,GLOBAL_LOG_TRACE)
	def thermostatData=[:]
	def listOfActiveSensors=[]
	def deviceData=[:]
	deviceData=atomicState?.deviceData
	if (!deviceData) {
		traceEvent(settings.logFilter,"getRCSActiveSensorsForTstat>No device data available, exiting...", detailedNotif,GLOBAL_LOG_WARN)
    	return []
	}        
	try {    
		thermostatData=deviceData[thermostatId]
		listOfActiveSensors= thermostatData?.active_rcs_sensors
		if ((listOfActiveSensors=="") || (listOfActiveSensors==null))  listOfActiveSensors=[]        
		traceEvent(settings.logFilter,"getRCSActiveSensorsForTstat>List of Active Sensors=$listOfActiveSensors for serial number $thermostatId...", detailedNotif,GLOBAL_LOG_INFO)
	} catch (e) {
		traceEvent(settings.logFilter,"getRCSActiveSensorsForTstat>no thermostat data for serial number $thermostatId...", detailedNotif,GLOBAL_LOG_WARN)
	}    
	return listOfActiveSensors    
}
// sensorsSN is a String containing the list of serial numbers separated by comma (ex. "kryptonite.18B430C283CA2593,kryptonite.18B430C283CA2595,kryptonite.18B430C283CA2596")
boolean setRCSActiveSensorsForTstat(String thermostatId, String sensorsSN) {
	
	boolean result=true
	traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>begin with $sensorsSN list",settings.trace,GLOBAL_LOG_INFO )
	def listSensors=[]    
	if (sensorsSN != null) {
		listSensors=sensorsSN.toString().minus('[').minus(']').split(",")

	}	    
	traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>list of sensors=$listSensors",settings.trace,GLOBAL_LOG_INFO )
	def sensorsWithPrefix=[]  
	listSensors.each {
		if ((it.toString() != "") && !(it.toString().contains("kryptonite."))) {    
			sensorsWithPrefix.add("kryptonite.${it}")\
		} else if (it.toString() != "") {
			sensorsWithPrefix.add(it)        
		}
	}        
	def listOfAllSensors= getRCSAssociatedSensorsForTstat(thermostatId)
	def listOfNonActiveSensors= listOfAllSensors - sensorsWithPrefix
     
	traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>About to look for thermostatId $thermostatId object in active devices ...", detailedNotif,GLOBAL_LOG_TRACE)
	def d= allChildDevices.find {it.deviceNetworkId.contains(thermostatId)}               
	if(d) {
		traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>found $d for $thermostatId, about to set $sensorsSN list active...", detailedNotif,GLOBAL_LOG_TRACE)
		d.setRCSActiveSensors(sensorsSN)
		...
		if ((exceptionCheck) && (exceptionCheck.contains("exception") || (exceptionCheck.contains("error")))) {
			traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>error while setting list of Active Sensors on thermostat $d", true,GLOBAL_LOG_ERROR)
			result=false        
		}        
	} else {
		traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>no active device found $d for $thermostatId...", detailedNotif,GLOBAL_LOG_TRACE)
		result=false				        
	} 
	listOfNonActiveSensors.each {		    
		def sensor_id = it?.tokenize('.')?.last()  
		traceEvent(settings.logFilter,"setRCSActiveSensorsForTstat>About to set sensor $sensor_id to be inactive (switched off)...", detailedNotif,GLOBAL_LOG_TRACE)
		d= allChildDevices.find {it.deviceNetworkId.contains(sensor_id)}               
		if(d) {
			d.sendEvent(name: "switch", value: "off")        
			d.sendEvent(name: "active_rcs_sensors", value: "[]")        
		}        
...
//	settings.logFilter=5   
	...
	if (useCache) {    
		...
		def cache_timestamp= (atomicState?."${type}_timestamp")            
		def cached_interval=(cache_timeout) ?:2  // set a minimum of caching to avoid unecessary load on Nest servers
		def time_check_for_cache = (now() - (cached_interval * 60 * 1000)).toLong()
		traceEvent(settings.logFilter,"getStructures>about to get structures from global cache $cache", detailedNotif)
		if ((atomicState?."${type}") && ((cache_timestamp) && (cache_timestamp.toLong() > time_check_for_cache))) {  //cache still valid
 			traceEvent(settings.logFilter,"cache_timestamp= $cache_timestamp, cache= ${cache}",detailedNotif)
		...
		if (type==GLOBAL_BUCKETS_CLAUSE) {    
			...
			if ((initialLoad) || (selectedInApp && child_devices) || (object_key.toString().startsWith("structure."))) {
				...
				if (object_key.toString().startsWith("shared.${sn}")) {
					...
					deviceData[sn] = (old_data)? (old_data + new_data): new_data                
					def sensors= (String []) object_data?.associated_rcs_sensors
					sensors.each {
						def sensor_id = it?.tokenize('.')?.last()  
						traceEvent(settings.logFilter, "getStructures>id = $sn, objectType=rcs_settings, linking tstat to sensors\n", detailedNotif,GLOBAL_LOG_TRACE)
						def sensor_data=[]
						try {
							sensor_data= deviceData[sensor_id]              
						} catch (e) { }
						deviceData[sensor_id]= sensor_data + ['tstat_id': sn] 
					}                    
...
def getSensorChildName() { "My Next SensorV3" }
